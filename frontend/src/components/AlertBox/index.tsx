...
export default function AlertBox(): React.ReactElement {
  ...
  return (
    <div className="fixed top-20 left-1/2 transform -translate-x-1/2 z-10">
      ...
      <div className={`transition duration-200 ease-in-out ${success ? 'opacity-100' : 'opacity-0'}`}>
        {!!success && <Success message={success.message} />}
      </div>
      <div className={`transition duration-200 ease-in-out ${loading ? 'opacity-100' : 'opacity-0'}`}>
        {loading && <Loading />}
      </div>
    </div>
  );
